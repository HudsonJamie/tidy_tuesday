...
data_2 <- data %>%
  ...
  do(add_row(
    .,
    install_year = setdiff(seq(1990, 2020, 1), data$install_year),
    .before = 0
  )) %>%
  arrange(install_year) %>%
  ungroup()
...
point_data <- map_df(
  ...
  ~ sigmoid(-0.2, 1.05, as.numeric(data_2[.x, 5]), as.numeric(data_2[.x, 6])) %>%
    ...
    bind_cols(bind_rows(replicate(
      100, data_2[.x,], simplify = FALSE
    )))
) %>%
...
anim <- ggplot() +
  geom_point(point_data,
             mapping = aes(x = x, y = y, colour = install_year),
             size = 3) +
  geom_label(
    point_data,
    hjust = 0,
    mapping = aes(
      label = paste("Current year:", frame),
      x = -0.8,
      y = 0.9
    ),
    color = "black",
  ...
  labs(
    title = "Water access points in five African countries",
...
anim_save(paste0("water_sources_", format(Sys.time(), "%d%m%Y"), ".gif"))
