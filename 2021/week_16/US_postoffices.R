...
state_size <- read.csv("https://raw.githubusercontent.com/jakevdp/data-USstates/master/state-areas.csv") # Read in state size data
...
spdf_fortified_size_post$bin_den <- cut( spdf_fortified_size_post$density*1000 , breaks=c(seq(0,50, 10), Inf), labels=c("0-10", "11-20", "21-30", "31-40", "41-50", "51+" ), include.lowest = TRUE )

(A <- ggplot() +
  geom_polygon(data = spdf_fortified_size_post, aes(fill =  bin_den, x = long, y = lat, group = group)) +
  scale_fill_viridis_d(option = "E", end = 0.9,
                       guide = guide_legend( keyheight = unit(5, units = "mm"), keywidth=unit(10, units = "mm"), 
                                             label.position = "right", 
                                             title.position = 'top', 
                                             nrow=6)) +
  geom_text(data=centers_join, aes(x=x, y=y, label=id),
            size = 3,
...
spdf_fortified_size_post$bin_rev <- cut( spdf_fortified_size_post$rev_density , breaks=c(seq(0,250, 50), Inf), labels=c("0-50", "51-100", "101-150", "151-200", "201-250", "251+" ), include.lowest = TRUE )

(B <- ggplot() +
  geom_polygon(data = spdf_fortified_size_post, aes(fill =  bin_rev, x = long, y = lat, group = group)) +
  scale_fill_viridis_d(option = "E", end = 0.9,
                       guide = guide_legend( keyheight = unit(5, units = "mm"), keywidth=unit(10, units = "mm"), 
                                             label.position = "right", 
                                             title.position = 'top', 
...
ggsave(paste0("us_post", format(Sys.time(), "%d%m%Y"), ".png"),
       dpi = 320,
       width = 10,
       height = 8.36,
       type = "cairo-png")
