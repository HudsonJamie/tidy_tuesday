...
# Created: 21 Apr 2021
# Edited: 21 Apr 2021
# Data: Shivam Bansal - Kaggle https://www.kaggle.com/shivamb/netflix-shows?select=netflix_titles.csv
...
top_genres <- tv_ratings %>%
  ...
  unnest(listed_in) %>%
  mutate(
    listed_in = str_remove_all(listed_in, c("TV|Movies|Series|Features|Shows")),
...
top_genres$listed_in_f <-
  factor(top_genres$listed_in, levels = top_genres$listed_in)
...
top_genres_RT$listed_in_f <-
  factor(top_genres_RT$listed_in, levels = top_genres_RT$listed_in)
