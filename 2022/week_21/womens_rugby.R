...
(goalposts <- ggplot(winlos_top10) +
    geom_segment(aes(yend = total_games), x = 0.65, xend = 0.65, y = 0,
                 size = 1.5, colour = "white", lineend = "round") +
    geom_segment(aes(yend = total_games), x = 1.35, xend = 1.35, y = 0,
                 size = 1.5, colour = "white", lineend = "round") +
    geom_segment( aes(y = total_loss, yend = total_loss), x = 0.65, xend = 1.35,
                  size = 1.5, colour = "white", lineend = "round") +
    geom_text(mapping = aes(x = 0.67, y = total_loss + 30, label = team),
              size = 5, hjust = 0,
...
(final_plot <- goalposts +
    ...
    inset_element(legend, left = 0.82, bottom = 1, right = 0.97, top = 1.32, align_to = 'full', clip = F))

ggsave(paste0("womens_rugby_", format(Sys.time(), "%d%m%Y"), ".png"),
       dpi = 320,
       width = 10,
       height = 7)
